...
function renderPdfPages(kp){
  ...
  const totalsAndConditions=()=>`<div class="kp-positions-totals"><div class="kp-positions-total-row"><div class="kp-positions-total-label strong">ИТОГО</div><div class="kp-positions-total-value strong">${formatMoneyWithRub(vm.totalSum)}</div></div>${vm.header.showVat?`<div class="kp-positions-total-row vat"><div class="kp-positions-total-label">ИТОГО С НДС 22%</div><div class="kp-positions-total-value">${formatMoneyWithRub(vm.totalVat)}</div></div>`:''}${Number(vm.header.deliveryPrice||0)>0?`<div class="kp-positions-total-row"><div class="kp-positions-total-label">Доставка и монтаж</div><div class="kp-positions-total-value">${formatMoneyWithRub(vm.header.deliveryPrice)}</div></div>`:''}${Number(vm.header.discountPercent||0)>0?`<div class="kp-positions-total-row"><div class="kp-positions-total-label">Скидка</div><div class="kp-positions-total-value">${vm.header.discountPercent}%</div></div><div class="kp-positions-total-row final"><div class="kp-positions-total-label">ИТОГО СО СКИДКОЙ</div><div class="kp-positions-total-value">${formatMoneyWithRub(vm.totalWithDiscount)}</div></div>`:''}</div><div class="kp-conditions"><p>Срок изготовления — ${vm.header.productionDays} календарных дней.</p><p>Указанная стоимость действительна в течение ${vm.header.offerValidDays} календарных дней со дня выставления счёта.</p><p>Условия оплаты: ${vm.header.prepaymentPercent}% предоплата. Остаток ${vm.header.finalPaymentPercent}% — по факту готовности изделий.</p>${Number(vm.header.deliveryPrice||0)>0?`<p>${escapeHtml(vm.header.deliveryTerms||'Сборка и доставка')} — ${formatMoneyWithRub(vm.header.deliveryPrice)}.</p>`:(vm.header.deliveryTerms?`<p>${escapeHtml(vm.header.deliveryTerms)}.</p>`:'')}</div>`;
  ...
  if(count<=10){
    ...
    html+=positionsPage(vm.items,10-count,true);
  }else if(count<=15){
    ...
    html+=positionsPage(finalItems,Math.max(0,10-finalItems.length),true);
  }else{
    ...
    html+=positionsPage(rest,Math.max(0,10-rest.length),true);
  }

  html+=`<div class="kp-page kp-final-page"><div class="kp-final-logo">${logo}</div><div class="kp-final-bottom"><div class="kp-final-left"><div>ПРОИЗВОДСТВО</div><div>МОСКВА, ШОССЕЙНАЯ 90С5</div><div style="margin-top:22px">САЛОН</div><div>ARTPLAY, ПО ЗАПИСИ</div></div><div class="kp-final-right"><div>МЕНЕДЖЕР ВАШЕГО ПРОЕКТА</div><div style="margin-top:6px">${escapeHtml(vm.header.manager||'')} ${escapeHtml(vm.header.managerPhone||'')}</div><div class="kp-final-site">forma-dom.com</div></div></div></div></div>`;
  return html;
}
